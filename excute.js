...
let 存入仓库数量 = 0;
var 挂机点跑图顺序 = 0;
var 上次装备自检时间 = new Date().getTime() - (20 * 60 * 1000); // 减去 20 分钟; 
var 上次设置内挂时间 = new Date().getTime() - (1000 * 60 * 1000); // 减去 1000 分钟;
var 上次设置攻击面板时间 = new Date().getTime() - (1000 * 60 * 1000); // 减去 1000 分钟;
var 上次设置分身面板时间 = new Date().getTime() - (1000 * 60 * 1000); // 减去 1000 分钟;
var 上次设置操作模式时间 = new Date().getTime() - (1000 * 60 * 1000); // 减去 1000 分钟;
var 内挂时间戳 = 1000 * 60 * 20;
var 攻击面板时间戳 = 1000 * 60 * 60;
var 分身面板时间戳 = 1000 * 60 * 60;
var 操作模式时间戳 = 1000 * 60 * 60;
var 上次挂机跑图坐标 = null;
var 上次挂机跑图坐标截图 = null;
var 上次坐标截图 = null;
var 启动金币 = "未知"
...
let window = floaty.window(
    <frame padding="2" id="xuanFuPanel" w="wrap_content" h="wrap_content">
        <horizontal>
            <text id="bbText" text="v:3.3.7" textSize="8sp" textColor="#ffffff" marginRight="3" />
...
var tools = {
    ...
    重启游戏: () => {
        ...
        while (true) {
            if (new Date().getTime() - start > timeout) {
                tools.发送邮件("重启游戏失败", "loginquedingBtn.png找不到");
                return false;
            }
        ...
        while (true) {
            if (new Date().getTime() - start > timeout) {
                tools.发送邮件("重启游戏失败", "kaishiyouxiBtn.png找不到");
                return false;
            }
            r = tools.findImageClick("kaishiyouxiBtn.png");
            if (r) {
                break;
            }
            sleep(2000);
        ...
        while (true) {
            if (new Date().getTime() - start > timeout) {
                tools.发送邮件("重启游戏失败", "kaishiyouxi.png找不到");
                return false;
            }
            r = tools.findImageClick("kaishiyouxi.png");
            if (r) {
                break;
            }
            sleep(2000);
        ...
        while (true) {
            tryCount++;
            if (new Date().getTime() - start > timeout) {
                tools.发送邮件("重启游戏失败", "yijianxiaoTuiBtn.png找不到");
                return false;
            }
    ...
    常用操作: {
        ...
        初始化挂机: () => {
            ...
            if (isStart && start - 上次设置内挂时间 >= 内挂时间戳) {
                tools.悬浮球描述("设置内挂参数");
                tools.常用操作.设置内挂();
                上次设置内挂时间 = start;
                tools.常用操作.关闭所有窗口();
            ...
            if (isStart && start - 上次设置攻击面板时间 >= 攻击面板时间戳) {
                tools.悬浮球描述("设置攻击面板");
                var r = null;
                while (isStart) {
            ...
            if (isStart && start - 上次设置分身面板时间 >= 分身面板时间戳) {
                tools.悬浮球描述("设置分身面板");
            ...
            if (isStart && start - 上次设置操作模式时间 >= 操作模式时间戳) {
                tools.悬浮球描述("设置操作模式");
                tools.常用操作.初始化操作模式(2);
                上次设置操作模式时间 = start;
            }
        ...
        点击挂机坐标: (是否强制跑图) => {
            ...
            if (是否强制跑图) {
                ...
                var r = tools.isSimilarImage(当前坐标截图, 上次挂机跑图坐标截图, 20, 6)
                if (r) {
        ...
        判断是否需要补给: () => {
            ...
            if (isStart && 挂机参数.衣服持久0回程 == 1 || 挂机参数.衣服持久0回程 == "1") {
                if (r && r.衣服 && r.衣服.剩持久 <= 2) {
                    return true;
                }
            }
            if (isStart && r && r.武器 && (r.武器.满持久 - r.武器.剩持久) >= 5) {
                var isOk = tools.喝修复油();
                if (!isOk) {
    ...
    人物移动: {
        ...
        去比奇老兵Loop: () => {
            ...
            var tryCount = 0;
            var 当前地图 = tools.常用操作.获取人物地图();
            var routesGroup = config.地图路由[当前地图]["回比奇老兵"];
            var 安全区坐标范围 = config.zuobiao.比奇安全区坐标范围;
        ...
        去盟重老兵Loop: () => {
            ...
            var tryCount = 0;
            var 当前地图 = tools.常用操作.获取人物地图();
            var routesGroup = config.地图路由[当前地图]["回盟重老兵"];
            var 安全区坐标范围 = config.zuobiao.盟重安全区坐标范围;
            for (var index = 0; index < routesGroup.length; index++) {
                while (isStart) {
                    当前地图 = tools.常用操作.获取人物地图();
        ...
        去挂机地图Loop: () => {
            ...
            var routesGroup = config.地图路由[当前地图][挂机参数.挂机地图];
            for (let index = 0; index < routesGroup.length; index++) {
                ...
                while (isStart) {
                    ...
                    if (上次坐标截图 == null) {
                        ...
                        var 当前坐标截图 = tools.常用操作.截图当前坐标();
                        var r = tools.isSimilarImage(当前坐标截图, 上次坐标截图, 20, 6)
                        if (r) {
                            utils.recycleNull(当前坐标截图);
                            是否跑图 = true;
                            tools.悬浮球描述("人物未移动")
        ...
        if (isShiQu) {
            ...
            while (isStart) {
                ...
                if (r.status) {
                    if ((挂机参数.是否隐身 == 1 || 挂机参数.是否隐身 == "1")) {
                        怪物 = tools.获取身边怪物数据();
                        if (new Date().getTime() - 上一次隐身 >= 隐身时间戳 && 怪物 && 怪物.length >= parseInt(挂机参数.隐身数量)) {
                            tools.常用操作.启动隐身();
    ...
    开始拾取: () => {
        ...
        while (isStart) {
            ...
            //tools.悬浮球临时描述(r)
            if ((r.indexOf("不能") >= 0 || r.indexOf("拾取") >= 0) && (r.indexOf("一定") >= 0 || r.indexOf("时间") >= 0)) {
                click(random(按钮集合.拾取.x[0], 按钮集合.拾取.x[1]), random(按钮集合.拾取.y[0], 按钮集合.拾取.y[1]));
                break;
            }
            if ((r.indexOf("不足") >= 0 || r.indexOf("无法使用") >= 0) && r.indexOf("魔法") >= 0) {
                if (挂机参数.无蓝等待 == 1) {
                    是否等待蓝回满 = true;
                }
                if (挂机参数.无蓝回城 == 1) {
                    ...
                    tools.错误日志("药不足，回城补给 " + r, 2);
                    toastLog("药不足，回城补给")
                }
            }
            if (r.indexOf("已满") >= 0 || r.indexOf("己满") >= 0 || r.indexOf("负重") >= 0) {
                if (挂机参数.装备实际未满下线 == 1) {
                    var r1 = tools.常用操作.检查背包是否有东西("5_7");
                    if (r1) {
                        tools.回城补给在挂机();
    ...
    while (true) {
        ...
        if (isStart) {
            ...
            if (new Date().getTime() - 上次装备自检时间 > 装备自检时间戳) {
                var r = tools.findImage("lianjiezhongduan.png");
                if (r.status) {
                    当前总状态 = 总状态.申请重启;
                    continue;
                }
                ocrPladderOCR.release();
                ocrPladderOCR = $ocr.create();//调用release()函数释放资源
                if (开启强行补给) {
                    toastLog("强制回城补给")
                    tools.回城补给在挂机();
                    开启强行补给 = false;
