            ...
            <vertical id="content" paddingLeft="6">
                ...
                <vertical id="view2" visibility="gone" gravity="center">
                    ...
                    <horizontal>
                        ...
                        <horizontal gravity="right">
                            <checkbox id="cbDiTuTuoDong" text="不打邪恶" textSize="10sp" />
                        </horizontal>
    ...
    挂机打怪: {
        ...
        攻击怪物: () => {
            ...
            while (当前总状态 == 总状态.已启动) {
                ...
                if (锁定失败次数 >= 3 || (挂机参数.不打邪恶 == 1 && (锁定的怪物.indexOf("邪") >= 0 || 左面板Str.indexOf("邪") >= 0))) {
                    tools.挂机打怪.关闭中间怪物();
                    tools.挂机打怪.强制跑图();
                    if (挂机参数.隐身走动 == 1) {
                        tools.常用操作.点击左面板怪物();
                    }
                    if (锁定的怪物.indexOf("邪") >= 0) {
                        toastLog("不打邪恶");
                ...
                if (r.status) {
                    ...
                    if (锁定的怪物.length > 0 && (!是否隐身等待 || 是否强制攻击) && (new Date().getTime() - 上一次怪物身边时间) >= 移动时间戳) {
                        ...
                        if (r.status) {
                            ...
                            if (r.status) {
                                ...
                                tools.挂机打怪.强制跑图();
                                上次跑图时间 = new Date().getTime();
                                tools.悬浮球临时描述("强制跑图2(" + ((t2 - t1) / 1000).toFixed(2) + ")")
                                //tools.悬浮球临时描述("跑图哦")
                                sleep(666)
                                锁定失败次数++;
                                break;
                    ...
                    //     }
                    // }
                    if (挂机参数.不打邪恶 == 0 && 精英怪 && 精英怪.status && !是否锁定危险怪) {
                        if (精英怪.value.是否施毒) {
                            tools.挂机打怪.施毒();
                    ...
                    if (挂机参数.不打邪恶 == 0 && 精英怪 && 精英怪.status && 精英怪.value.是否施毒) {
                            ...
                            if ((!是否隐身等待 || 是否强制攻击) && 锁定的怪物.length <= 0) {
                                if (挂机参数.隐身走动 == 0) {
                                    ...
                                    if (r.status) {
                                        ...
                                        tools.挂机打怪.强制跑图();
                                        上次跑图时间 = new Date().getTime();
                                        tools.悬浮球临时描述("强制跑图1(" + ((t2 - t1) / 1000).toFixed(2) + ")")
                                        sleep(666)
                                        锁定失败次数++;
                                        break;
                                    }
                    ...
                    if (挂机参数.不打邪恶 == 0 && 挂机参数.召唤宝宝 == 1 && new Date().getTime() - 上次检查宝宝时间 > 检查宝宝时间戳 && (锁定的怪物.indexOf("邪") >= 0 || 左面板Str.indexOf("邪") >= 0)) {
        ...
        判断是否强制攻击: () => {
            ...
            var img = captureScreen();
            var result = tools.挂机打怪.找非满血怪(img, false, false);//注意这里必须提前切换左面板选怪人物
            utils.recycleNull(img);
            if (result.status) {
...
ui.run(() => {
    ...
    win.btnSave.click(() => {
        ...
        挂机参数 = {
            ...
            不打邪恶: win.cbDiTuTuoDong.isChecked() ? 1 : 0,
