...
var tools = {
    ...
    挂机打怪: {
        ...
        攻击怪物: () => {
            ...
            var 扫描怪物名失败次数 = 0;
            var 扫描怪物名时间戳 = 1000 * 1;
            var 上一次移动扫描怪物名时间 = new Date().getTime() - (60 * 1000);
            ...
            while (当前总状态 == 总状态.已启动) {
                ...
                if (r.status) {
                    ...
                    if (身边怪物.length <= 0 || (new Date().getTime() - 上一次移动扫描怪物名时间 >= 扫描怪物名时间戳)) {
                        ...
                        if (扫描R.status && 扫描R.name.length > 0) {
                            ...
                            扫描怪物名失败次数++;
                            toastLog("扫描身边怪物名失败(" + 锁定的怪物 + ")[" + 扫描怪物名失败次数 + "]")
                        }
                    }

                    // var 宝宝身边 = tools.挂机打怪.获取宝宝身边怪物数据(1);
                    // tools.悬浮球描述("宝宝(" + 宝宝身边.value.length + ")")
                    if ((!是否隐身等待 || 是否强制攻击) && 扫描怪物名失败次数 >= 3) {
                        r = tools.挂机打怪.找正上锁定怪物(0, 0);
                        if (r.status) {
                            var t1 = new Date().getTime();
                    ...
                    左面板Str = (左面板怪物 != null && 左面板怪物.status ? 左面板怪物.value.name : "null");
            ...
            if (宝宝身边怪物.status && 宝宝身边怪物.value && 宝宝身边怪物.value.length > 0) {
                ...
                for (var index = 0; index < result.length; index++) {
                    ...
                    if (r.x >= 245 && r.x <= 915 && r.y >= 60 && r.y <= 556) {
                        tools.click(r.x + random(10, 20), r.y + random(1, 5));
                        sleep(222);
                        r = tools.挂机打怪.找正上锁定怪物(0, 0);
                        if (r.status) {
                            if (是否攻击) {
